...
export async function PUT(request: Request, context: RouteParams) {
  ...
  if (!id) {
    return jsonError(400, "Invalid handout ID.");
  }
  ...
  try {
    const payload = await ctx.handoutService.updateHandout({
    ...
    return toServiceErrorResponse(err, "Could not update handout.");
  }
}
...
export async function DELETE(request: Request, context: RouteParams) {
  ...
  try {
    const payload = await ctx.handoutService.deleteHandout({
      req: ctx.toReqLike(auth.payload, request),
      id,
      actorUsername: auth.payload.session.username,
      isAdmin: auth.payload.session.role === "admin",
    ...
    return toServiceErrorResponse(err, "Could not delete handout.");
  }
}
