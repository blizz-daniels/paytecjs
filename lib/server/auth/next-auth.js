...
const isProduction = normalizeNodeEnv(process.env.NODE_ENV) === "production";
...
function getRuntimeState() {
  ...
  if (runtimeIsProduction && databaseRuntime.driver !== "postgres") {
    throw new Error("Next auth sessions require Postgres in production.");
  }

  runtimeState = {
    nodeEnv,
    isProduction: runtimeIsProduction,
    dataDir,
...
async function run(sql, params = []) {
  return getRuntimeState().db.run(sql, params);
}

async function get(sql, params = []) {
  return getRuntimeState().db.get(sql, params);
}

async function all(sql, params = []) {
  return getRuntimeState().db.all(sql, params);
}

async function withSqlTransaction(work) {
  return getRuntimeState().db.transaction(work);
}
