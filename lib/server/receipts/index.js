function createReceiptService(options = {}) {
  ...
  async function upsertApprovedReceiptFromTransaction(transactionId, optionsInput = {}) {
    ...
    try {
      if (!fs.existsSync(syntheticPath)) {
        await fs.promises.writeFile(syntheticPath, `Auto-generated placeholder for approved transaction #${id}.\n`, "utf8");
      }
    } catch (_err) {
  ...
  async function resolveApprovedReceiptFileAccess(input = {}) {
    ...
    const approvedPathValid = isPathInsideDirectory(approvedReceiptsDir, absoluteApprovedPath) && fs.existsSync(absoluteApprovedPath);
    if (!approvedPathValid) {
      ...
      if (delivery && (delivery.error || Number(delivery.failed || 0) > 0)) {
        logger.error(
          `[approved-receipts] on-demand regeneration failed payment_receipt_id=${receiptId} reason=${
            delivery.error || "unknown"
          }`
        );
      }
    ...
    } else if (!input.refreshRequested && isLikelyLegacyPlainReceipt(absoluteApprovedPath)) {
      const delivery = await triggerApprovedReceiptDispatchForReceipt(receiptId, {
        actorUsername: input.actorUsername || "system-download",
        forceEnabled: true,
        forceRegenerate: true,
      ...
      if (delivery && (delivery.error || Number(delivery.failed || 0) > 0)) {
        logger.error(
          `[approved-receipts] auto-upgrade regeneration failed payment_receipt_id=${receiptId} reason=${
            delivery.error || "unknown"
          }`
        );
      }
